...
def update_value(list_result: list, grid_range_to_update: str, gsheet_id: str):
    ...
    """
    body = {"values": list_result}  # list_result is array 2 dimensional (2D)
    result = (
        service()
        .spreadsheets()
        .values()
...
def get_df_from_speadsheet(gsheet_id: str, sheet_name: str):
    ...
    k = [None] * x
    check_fistrow.extend(
        k
    )  # if only have column name but all data of column null =>> error
    row = data[2:]
...
def creat_new_sheet_and_update_data_from_df(
    df: object, gsheet_id: str, new_sheet_name: str
):
...
def create_new_gsheet(new_gsheet_title: str):
    spreadsheet = {"properties": {"title": new_gsheet_title}}
    spreadsheet = (
        service()
        .spreadsheets()
    ...
    print(
        "https://docs.google.com/spreadsheets/d/{0}".format(
...
def insert_column(gsheet_id, sheet_name: str, n_column_to_append: int):
    sheet_id = get_sheet_id_from_gsheet_id_and_sheet_name(
        gsheet_id=gsheet_id, sheet_name=sheet_name
    )
    try:
        request_body = {
            "requests": [
...
def update_value_at_last_column(
    df_to_update: object, gsheet_id: str, sheet_name: str, start_row: int = 1
):
    ...
    list_result = df_to_update.values.tolist()  # transfer data_frame to 2D list
    list_result.insert(0, df_to_update_columns)
    grid_range_to_update = (
        f"{sheet_name}!{colnum_string(last_colum_index+1)}{start_row}"
    )
    update_value(
        gsheet_id=gsheet_id,
