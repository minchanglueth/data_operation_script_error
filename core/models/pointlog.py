...
class PointLog(Base, TimestampMixin):
    ...
    info = sa.Column(
        "Info", MutableDict.as_mutable(sa.JSON), default=None, nullable=False
    )
    verified_info = sa.Column(
        "VerifiedInfo", MutableDict.as_mutable(sa.JSON), default=None, nullable=False
    )
