...
def count_datasource_by_artistname_formatid(artist_name: str, formatid: str):
    count_datasource_by_artistname_formatid = (
        ...
        .filter(
            or_(
                ItunesRelease.album_artist.like("%" + artist_name + "%"),
...
def get_datasource_by_artistname_formatid(artist_name: str, formatid: str):
    get_datasource_by_artistname_formatid = (
        db_session.query(
            ...
            ItunesRelease.duration,
            (
                extract("hour", ItunesRelease.duration) * 3600000
                + extract("minute", ItunesRelease.duration) * 60000
                + extract("second", ItunesRelease.duration) * 1000
            ).label("DurationMs"),
            Track.id.label("trackid"),
...
def get_crawlingtask_youtube_info(objectid: str, PIC: str, actionid: str):
    get_crawlingtask_info = (
        ...
        .filter(
            Crawlingtask.objectid == objectid,
...
def get_crawlingtask_info(objectid: str, PIC: str, actionid: str):
    ...
    get_crawlingtask_info = (
        db_session.query(
            Crawlingtask.id,
            Crawlingtask.objectid,
            func.json_extract(Crawlingtask.taskdetail, f"$.{url}").label("url"),
            func.json_extract(Crawlingtask.taskdetail, "$.when_exists").label(
                "when_exists"
            ),
            Crawlingtask.status,
...
def get_crawlingtask_status(gsheet_name: str, sheet_name: str, actionid: str):
    ...
    crawl_artist_image_status = (
        db_session.query(
            Crawlingtask.id,
            Crawlingtask.objectid,
            func.json_extract(Crawlingtask.taskdetail, f"$.{url}").label(f"{url}"),
            func.json_extract(Crawlingtask.taskdetail, "$.when_exists").label(
                "when_exists"
            ),
            Crawlingtask.status,
...
def get_s11_crawlingtask_info(pic: str):
    ...
    s11_crawlingtask_info = (
        (
            db_session.query(
                func.json_extract(crawlingtasks_06.taskdetail, f"$.album_id").label(
                    "itune_album_id"
                ),
        ...
        .order_by(crawlingtasks_06.created_at.desc())
    )
...
def get_track_title_track_artist_by_ituneid_and_seq(itune_album_id: str, seq: str):
    get_track_title_track_artist_by_ituneid_and_seq = (
        (
            db_session.query(Track.title, Track.artist, Track.id, Track.duration_ms)
            .select_from(ExternalIdentity)
            .join(Album, Album.uuid == ExternalIdentity.uuid)
            .join(
                Album_Track,
                Album.uuid == Album_Track.album_uuid,
            )
            .join(Track, and_(Album_Track.track_id == Track.id, Track.valid == 1))
            .filter(
                Album.valid == 1,
...
def get_pointlogsid_valid(pointlogids: list):
    point_log_valid = (
        db_session.query(PointLog.id)
        .select_from(PointLog)
        .filter((PointLog.valid == 0), PointLog.id.in_(pointlogids))
...
def get_pointlogsid_valid_for_contribution(pointlogids: list):
    point_log_valid = (
        db_session.query(PointLog.id, PointLog.valid)
        .select_from(PointLog)
        .filter((PointLog.valid == 0), PointLog.id.in_(pointlogids))
        .order_by(PointLog.created_at.asc())
    )
