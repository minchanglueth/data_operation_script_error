...
def get_uuid_and_count_from_artist_name(artist_name: list):
    artist_uuid_from_artist_name = (
        db_session.query(
            Artist.name.label("artist_name"),
            Artist.uuid.label("artist_uuid"),
            func.count(Artist.uuid).label("count_uuid_by_name"),
        )
        .select_from(Artist)
        .filter(Artist.valid == 1, Artist.name.in_(artist_name))
...
def get_all_by_ids(artist_uuids: list):
    return (
        ...
        .filter((Artist.valid == 1), Artist.uuid.in_(artist_uuids))
...
def get_one_by_id(artist_uuid: str):
    return (
        db_session.query(Artist)
        .filter((Artist.valid == 1), Artist.uuid == artist_uuid)
        .order_by(Artist.created_at.desc())
        .first()
    )
...
if __name__ == "__main__":
    ...
    artist_uuid = "8A584B04D8094A47AAC757252B9FD4AA"
    joy = get_compiled_raw_mysql(get_one_by_id(artist_uuid))
