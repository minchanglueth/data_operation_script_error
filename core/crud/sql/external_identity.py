...
def get_artists_from_album_ituneid(album_ituneid: list):
    sub_query = (
        db_session.query(ExternalIdentity.uuid.label("album_uuid"))
        .distinct(ExternalIdentity.uuid)
    ...
    artists_from_album_ituneid = (
        ...
        .select_from(Album)
        .outerjoin(
            Artist_album, (Artist_album.album_id == Album.id) & (Album.valid == 1)
        )
        .outerjoin(Artist, (Artist.id == Artist_album.artist_id) & (Artist.valid == 1))
        .outerjoin(
            ExternalIdentity,
            (ExternalIdentity.uuid == Artist.uuid) & (Artist.valid == 1),
        )
...
def get_trackid_from_ituneid_and_tracknum(album_ituneid: list, track_num: list):
    trackid_from_ituneid_and_tracknum = (
        db_session.query(
            ExternalIdentity.external_id.label("ituneid"),
        ...
        .outerjoin(ItunesRelease, (ItunesRelease.album_uuid == ExternalIdentity.uuid))
        .outerjoin(
            Track,
        ...
        .filter(
            ExternalIdentity.external_id.in_(album_ituneid),
            ItunesRelease.track_seq.in_(track_num),
        )
    )
...
if __name__ == "__main__":
    ...
    track_num = [1, 2]
    joy = get_compiled_raw_mysql(
        get_trackid_from_ituneid_and_tracknum(album_ituneid, track_num)
    )
    print(joy)
