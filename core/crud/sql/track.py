...
def get_track_lyric(trackid: tuple):
    track_lyrics = (
        db_session.query(Track.id, Track.title, Track.artist, Track.lyrics)
        .select_from(Track)
        .filter(Track.id.in_(trackid))
    )
    return track_lyrics
...
def get_all_by_track_ids(trackids: list):
    return (
        ...
        .filter((Track.valid == 1), Track.id.in_(trackids))
...
def get_one_track_by_id(track_id: str):
    db_track = (
        db_session.query(Track).filter((Track.valid == 1), Track.id == track_id).all()
    )
    if not db_track:
        return db_track
    else:
