...
def collect_from_youtube_query():
    collect_from_youtube_query = (
        db_session.query(
            PointLog.created_at,
            PointLog.valid,
            PointLog.id,
            func.json_extract(PointLog.info, "$.email").label("email"),
            func.json_extract(PointLog.info, "$.youtube_url").label("contribution_url"),
            ...
            func.json_extract(PointLog.info, "$.comment").label("contribution_comment"),
            PointLog.crawler_status,
            func.json_extract(PointLog.ext, "$.crawler_id").label("crawler_id"),
            Crawlingtask.status,
            PointLog.action_type,
            func.json_extract(DataSource.info, "$.source.title").label("youtube_title"),
        ...
        .outerjoin(
            Crawlingtask, text("crawlingtasks.id = pointlogs.ext ->> '$.crawler_id'")
        )
        .outerjoin(Album_Track, (Album_Track.track_id == PointLog.target_id))
        .outerjoin(Album, (Album.uuid == Album_Track.album_uuid) & (Album.valid == 1))
        .outerjoin(
            Track,
...
def get_cutoff_date_collect_from_youtube():
    get_date = (
        db_session.query(PointLog.created_at)
        .select_from(PointLog)
        .filter(PointLog.valid != 0, PointLog.action_type == "CY")
        .order_by(PointLog.created_at.desc())
        # .first()
    )
    return get_date
