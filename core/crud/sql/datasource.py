...
def get_datasourceids_from_youtube_url_and_trackid(
    youtube_url: str, trackid: str, formatid: str
):
    datasourceid = (
        db_session.query(DataSource.id)
...
def related_datasourceid(datasourceid: str):
    ...
    datasourceid = (
        db_session.query(
            ...
            PlaylistDataSource.playlist_id,
            UserNarrative.id.label("narrative_id"),
            CollectionDataSource.collection_uuid,
        )
        .select_from(DataSource)
        .outerjoin(
            PlaylistDataSource, PlaylistDataSource.datasource_id == DataSource.id
        )
        .outerjoin(
            UserNarrative, UserNarrative.content_json.like("%" + DataSource.id + "%")
        )
        .outerjoin(
            CollectionDataSource, CollectionDataSource.datasource_id == DataSource.id
        )
...
def get_all_datasource_valid() -> List[DataSource]:
    return (
        ...
        .filter(
            (DataSource.valid == 1),
            DataSource.format_id == "74BA994CF2B54C40946EA62C3979DDA3",
        )
...
def get_all_datasource_by_ids(datasourceids: list):
    ...
    return (
        db_session.query(DataSource)
        .filter((DataSource.valid == 1), DataSource.id.in_(datasourceids))
...
def get_one_datasource_by_id(datasourceid: str):
    ...
    db_datasource = (
        db_session.query(DataSource)
        .filter((DataSource.valid == 1), DataSource.id == datasourceid)
        .order_by(DataSource.updated_at.asc())
        .all()
    )
...
def get_youtube_info_from_trackid(track_ids: list, format_id):
    datasourceid = (
        (
            db_session.query(
                DataSource.track_id.label("track_id"),
            ...
            .select_from(DataSource)
            .filter(
                DataSource.track_id.in_(track_ids),
                DataSource.valid == 1,
                DataSource.format_id == format_id,
            )
        )
        .group_by(DataSource.track_id)
        .order_by(DataSource.created_at.desc())
    )
    return datasourceid
...
def get_youtube_title_and_youtube_uploader_from_youtube_url(youtube_url: str):
    datasourceid = (
        (
            ...
            .select_from(DataSource)
            .filter(DataSource.source_uri == youtube_url, DataSource.valid == 1)
        )
        .group_by(DataSource.track_id)
        .order_by(DataSource.created_at.desc())
    )
...
def get_list_datasourceid():
    datasourceid = (
        db_session.query(DataSource.id)
        .filter(DataSource.valid == 1, DataSource.updated_at > "2020-10-01")
