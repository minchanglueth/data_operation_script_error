...
def automate_check_crawl_image_status(gsheet_name: str, sheet_name: str):  # need to optimize
    # commit_message = input(f"\n Do you complete crawling_tasks insertion ?: True or False:")

    # if commit_message == '1':
    count = 0
    while True and count < 300:
        df1 = get_df_from_query(get_crawl_image_status(gsheet_name=gsheet_name, sheet_name=sheet_name))
        result = df1[
                     (df1.status != 'complete')
