...
def youtube_check_box(page_name: str, df: object, sheet_name: str):
    ...
    if (
        page_name in ("TopSingle", "TopAlbum")
        and sheet_name == SheetNames.MP3_SHEET_NAME
    ):
        ...
        youtube_check_box = df[
            ~(
                ...
                | (
                    ...
                    & (df["url_to_add"] == "")
                    & (df["type"] == "")
                    & ~((df["checking_mp3"] == "TRUE") & (df["is_released"] == "TRUE"))
                )
                | (
                    (df["track_id"] != "")
                    & (df["memo"] == "not found")
        ...
        youtube_check_box = df[
            ~(
                ...
                | ((df["track_id"] != "") & (df["memo"] == "added") & (df["len"] == 43))
                | (
                    (df["track_id"] != "")
                    & (df["memo"] == "not found")
                    & (df["url_to_add"] == "none")
...
def update_c11_check_box(original_df: object, pre_valid: str):
    original_df["url"] = original_df.apply(
        lambda x: get_key_value_from_gsheet_info(
            gsheet_info=x["gsheet_info"], key="url"
        )
    ...
    original_df["itune_id"] = original_df.apply(
        ...
        if (x["itune_album_url"] != "" and x["pre_valid"] == pre_valid)
        else x["itune_id"],
        axis=1,
    )
    original_df["region"] = original_df.apply(
        lambda x: get_itune_id_region_from_itune_url(url=x["itune_album_url"])[1]
        if (x["itune_album_url"] != "" and x["pre_valid"] == pre_valid)
        else x["region"],
        axis=1,
    )
    ...
    original_df["checking_validate_itune"] = original_df.apply(
        lambda x: check_validate_itune(
            itune_album_id=x["itune_id"], itune_region=x["region"]
        )
        if (x["itune_album_url"] != "" and x["pre_valid"] == pre_valid)
        else x["checking_validate_itune"],
        axis=1,
    )
